...
$cacheFileName = 'url_cache_cqyx_all.json';
$cachedUrls = [];
...
$playurl = "http://hls.live.cqccn.com/__cl/cg:live/__c/{$n[$id]}/__op/default/__f/index.m3u8";

$hosts = [
    'hls.live.cqccn.com:80:118.24.228.117'
];
...
if (isset($cachedUrls[$id])) {
    ...
    do {
        $finalUrl = get_data($playurl, $headers, $hosts);
        $domain = parse_url($finalUrl)['host'];

        if (in_array($domain, $channels)) {
            $playurl = preg_replace("/{$domain}/", "hls.live.cqccn.com", $finalUrl);
        } else {
            ob_clean();
            $cachedUrls[$id] = [
                'url' => $finalUrl
